...
from pygame.locals import (
    QUIT, KEYUP, KEYDOWN, K_ESCAPE, K_p, K_LEFT, K_RIGHT, K_DOWN, K_UP, K_SPACE
)
import constantes
import random
import time
...
# Classe Tetris
class Jeu:
    '''Classe principale du jeu Tetris et qui va gérer le jeu'''
    ...
    def calculer_donnees_piece_courante(self) -> None:
        ...
        for x, l in enumerate(m):
            for y, k in enumerate(l):
                if k != 0:
                    coords.append([x + self.position[0], y + self.position[1]
...
#		print("Position testée valide: x=%s, y=%s" % (x, y))
    ...
    def gerer_evenements(self) -> None:  #controleur
        ...
        if event == K_p:
            ...
            self.afficher_texte("Pause",
                                constantes.CENTRE_FENETRE,
                                font="titre")
            self.afficher_texte("Appuyer sur une touche...", constantes.POS)
            self.attente()
        elif event == K_LEFT:
            ...
            print(f"Mouvement de chute %s / %s {(self.position, self.coordonnees)}")
            if self.position[1] <= 0:
                self.position[1] = 1
                self.calculer_donnees_piece_courante()
            a = 0
    ...
    def dessiner_plateau(self) -> None:
        ...
        for i, ligne in enumerate(self.plateau):
            for j, case in enumerate(ligne):
                ...
                position = j, i
                coordonnees = tuple(
                    constantes.START_PLATEAU[k] +
                    position[k] * constantes.TAILLE_BLOC[k] for k in range(2)
                )
                pygame.draw.rect(self.surface, couleur,
                                 coordonnees + constantes.TAILLE_BLOC)
        if self.current is not None:
            for position in self.coordonnees:
                couleur = constantes.COULEURS.get(
                    self.get_current_piece_color())
                coordonnees = tuple(
                    constantes.START_PLATEAU[k] +
                    position[k] * constantes.TAILLE_BLOC[k] for k in range(2)
                )
        ...
        self.afficher_texte(f"Score: >{self.score}", constantes.POSITION_SCORE)
        self.afficher_texte(f"Pièces: {self.pieces}", constantes.POSITION_PIECES)
        self.afficher_texte(f"Lignes: {self.lignes}", constantes.POSITION_LIGNES)
        self.afficher_texte(f"Tetris: {self.tetris}", constantes.POSITION_TETRIS)
        self.afficher_texte(f"Niveau: {self.niveau}", constantes.POSITION_NIVEAU)
